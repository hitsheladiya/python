...
class HrPayrollExtend(HrPayroll):
    ...
    def get_payroll_report_print(self, list_ids='', **post):
        ...
        if employee_ids and not request.env.user.has_group("hr_payroll.group_hr_payroll_manager"):
            password = employee_ids[0].report_password
            if password:
